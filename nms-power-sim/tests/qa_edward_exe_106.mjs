#!/usr/bin/env node
/**
 * tests/qa_edward_exe_106.mjs — QA Edward 独立便携版验证（1.0.6）
 * 基于 qa_edward_exe_105.mjs 适配：exe 路径 / 版本标记升级为 1.0.6 性能修复
 * （engine 动态修订号 bumpDyn/getDynRev + board dynCache 门控 / scheduleRender 合帧），
...
const TMP_DIR = 'C:/Users/liao9/AppData/Local/Temp/QA 便携验证 106';
...
async function main() {
  ...
  if (s1.page) {
    ...
    ok('V3b 打包 renderer/engine.js 含结构修订号 + 动态修订号机制（bumpDyn/getDynRev）', rd.engine);
    ok('V3c 打包 renderer/board.js 含 1.0.6 门控（dynCache/refreshPowerState/scheduleRender）', rd.board);
    ...
    const entry = JSON.stringify([{ name: 'QAexe106写入', savedAt: new Date().toISOString(), count: 1, data: { elements: [], wires: [], simTime: 0, timeOfDay: 8, dayCycle: false, wireStyle: 'straight' } }]);
    const before = lsSnapshot();
    const w1 = await evalIn(s1.page, `localStorage.setItem('nmsPowerSimLibrary.v1', ${JSON.stringify(entry)}); return (localStorage.getItem('nmsPowerSimLibrary.v1') || '').includes('QAexe106写入');`);
    ...
    if (s2.page) {
      ...
      ok('V6 重启后实例数据仍可读（持久化实证）', typeof back === 'string' && back.includes('QAexe106写入'), `value=${String(back).slice(0, 90)} ${err ? 'err=' + err : ''}`);
    } else {
  ...
  const pass = R.fail === 0;
  console.log(pass ? 'QA_EXE_106_PASS' : 'QA_EXE_106_FAILED');
