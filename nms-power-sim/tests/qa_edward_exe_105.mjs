#!/usr/bin/env node
/**
 * tests/qa_edward_exe_105.mjs — QA Edward 独立便携版验证（1.0.5）
 ...
 * 1.0.5 版本号仅出现在原生「关于」对话框（DOM 不可探），改为核对：
 *   打包进 exe 的 renderer 副本含 1.0.5 性能修复源码标记 + 构建时序一致。
 */
...
const SRC_EXE = 'C:/Users/liao9/WorkBuddy/我的工作1/nms-power-sim-desktop/dist/无人深空电力模拟器-1.0.5-portable.exe';
const MOJ_JSON = 'C:/Users/liao9/WorkBuddy/我的工作1/moj-scroll-screen.json';
const TMP_DIR = 'C:/Users/liao9/AppData/Local/Temp/QA 便携验证 105';
const EXE = path.join(TMP_DIR, '无人深空电力模拟器-1.0.5-portable.exe');
...
async function main() {
  ...
  ok('V0c 源 exe 大小=108,766,218 字节', st.size === 108766218, String(st.size));
  ...
  if (s1.page) {
    // 版本断言：打包 renderer 含 1.0.5 性能修复源码标记 + 与源码哈希一致 + 构建时序
    ...
    ok('V3b 打包 renderer/engine.js 含结构修订号机制（bumpRevision/getRevision/keysRevision）', rd.engine);
    ok('V3c 打包 renderer/board.js 用 getRevision/hasWire 且已移除 computeStructureSig', rd.board);
    ...
    const entry = JSON.stringify([{ name: 'QAexe105写入', savedAt: new Date().toISOString(), count: 1, data: { elements: [], wires: [], simTime: 0, timeOfDay: 8, dayCycle: false, wireStyle: 'straight' } }]);
    const before = lsSnapshot();
    const w1 = await evalIn(s1.page, `localStorage.setItem('nmsPowerSimLibrary.v1', ${JSON.stringify(entry)}); return (localStorage.getItem('nmsPowerSimLibrary.v1') || '').includes('QAexe105写入');`);
    ...
    if (s2.page) {
      ...
      ok('V6 重启后实例数据仍可读（持久化实证）', typeof back === 'string' && back.includes('QAexe105写入'), `value=${String(back).slice(0, 90)} ${err ? 'err=' + err : ''}`);
    } else {
  ...
  const pass = R.fail === 0;
  console.log(pass ? 'QA_EXE_105_PASS' : 'QA_EXE_105_FAILED');
