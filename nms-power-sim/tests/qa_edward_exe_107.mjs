#!/usr/bin/env node
/**
 * tests/qa_edward_exe_107.mjs — QA Edward 独立便携版验证（1.0.7）
 * 基于 qa_edward_exe_106.mjs 适配：exe 路径 / 版本标记升级为 1.0.7 增量结构更新
 * （engine changesLog/getChangesSince 变更清单 + board applyIncremental /
 *   __nmsRenderStats 机制计数 + INC_MAX_BATCH 批量上限），
 * exe 体积断言放宽为合理区间（打包产物体积随版本浮动）。
 * 流程：复制 exe 到中文+空格目录 → 启动（--disable-gpu --no-sandbox + CDP）→
 *   存活/数据目录断言 → 大 JSON 导入流畅性 → CDP 写入实例库 → 优雅退出 →
 *   leveldb 落盘核对 → 重启复读（持久化实证）→ %APPDATA% 无新增写入 → 清理。
 */
...
const SRC_EXE = 'C:/Users/liao9/WorkBuddy/我的工作1/nms-power-sim-desktop/dist/无人深空电力模拟器-1.0.7-portable.exe';
const MOJ_JSON = 'C:/Users/liao9/WorkBuddy/我的工作1/moj-scroll-screen.json';
const TMP_DIR = 'C:/Users/liao9/AppData/Local/Temp/QA 便携验证 107';
const EXE = path.join(TMP_DIR, '无人深空电力模拟器-1.0.7-portable.exe');
...
async function main() {
  ...
  if (s1.page) {
    // 版本断言：打包 renderer 含 1.0.5+1.0.6 性能修复源码标记 + 与源码哈希一致 + 构建时序
    const rdRoot = 'C:/Users/liao9/WorkBuddy/我的工作1/nms-power-sim-desktop/renderer';
    const srcRoot = 'C:/Users/liao9/WorkBuddy/我的工作1/nms-power-sim';
    let rd = { engine: false, board: false, seq: false, hashEq: null };
    try {
      const engine = readFileSync(path.join(rdRoot, 'js/engine.js'), 'utf8');
      const board = readFileSync(path.join(rdRoot, 'js/board.js'), 'utf8');
      const exeM = statSync(SRC_EXE).mtimeMs;
      rd = {
        engine: engine.includes('bumpRevision') && engine.includes('getRevision') && engine.includes('keysRevision')
          && engine.includes('bumpDyn') && engine.includes('getDynRev')
          && engine.includes('getChangesSince') && engine.includes('changesLog'),
        board: board.includes('getRevision') && board.includes('hasWire') && !board.includes('function computeStructureSig')
          && board.includes('dynCache') && board.includes('refreshPowerState') && board.includes('scheduleRender')
          && board.includes('applyIncremental') && board.includes('getChangesSince') && board.includes('__nmsRenderStats')
          && board.includes('INC_MAX_BATCH'),
        seq: ['js/engine.js', 'js/board.js', 'js/app.js', 'index.html'].every((f) => statSync(path.join(rdRoot, f)).mtimeMs <= exeM),
      };
      const crypto = await import('node:crypto');
      rd.hashEq = ['js/engine.js', 'js/board.js'].every((f) =>
        crypto.createHash('sha256').update(readFileSync(path.join(rdRoot, f))).digest('hex') ===
        crypto.createHash('sha256').update(readFileSync(path.join(srcRoot, f))).digest('hex'));
    } catch (_) {}
    ok('V3b 打包 renderer/engine.js 含结构修订号 + 动态修订号 + 变更清单机制（bumpDyn/getDynRev/getChangesSince）', rd.engine);
    ok('V3c 打包 renderer/board.js 含 1.0.6 门控 + 1.0.7 增量更新（dynCache/applyIncremental/__nmsRenderStats）', rd.board);
    ok('V3d 打包 renderer 与源码引擎文件哈希一致（同步无漂移）', rd.hashEq === true);
    ok('V3e renderer 修改时间 ≤ exe 构建时间（同一批次构建）', rd.seq);

    // 大 JSON 导入流畅性（376 元件 / 620 导线，3s 内完成渲染）
    let importOk = null, importMs = -1;
    try {
      const qaJson = 'C:/Users/liao9/AppData/Local/Temp/qa105_moj_exe.json';
    ...
    const entry = JSON.stringify([{ name: 'QAexe107写入', savedAt: new Date().toISOString(), count: 1, data: { elements: [], wires: [], simTime: 0, timeOfDay: 8, dayCycle: false, wireStyle: 'straight' } }]);
    const before = lsSnapshot();
    const w1 = await evalIn(s1.page, `localStorage.setItem('nmsPowerSimLibrary.v1', ${JSON.stringify(entry)}); return (localStorage.getItem('nmsPowerSimLibrary.v1') || '').includes('QAexe107写入');`);
    ...
    if (s2.page) {
      ...
      ok('V6 重启后实例数据仍可读（持久化实证）', typeof back === 'string' && back.includes('QAexe107写入'), `value=${String(back).slice(0, 90)} ${err ? 'err=' + err : ''}`);
    } else {
  ...
  const pass = R.fail === 0;
  console.log(pass ? 'QA_EXE_107_PASS' : 'QA_EXE_107_FAILED');
