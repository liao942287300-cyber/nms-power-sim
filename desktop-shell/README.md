...
双击 `dist\无人深空电力模拟器-1.0.0-portable.exe` 即可（首次启动稍慢，属正常）。
...
| 视图 | 内容 |
| --- | --- |
| 元件图鉴 | 10 个电力元件的原理、端口、用法 |
| 仿真实验室 | 元件库 + 电路画布 + 实时状态面板；可放置/连线/开关/导出导入 |
| 应用实例 | 6 个直接来自教程素材的电路，一键「载入实验室」上电运行 |
...
`verify-cdp.mjs` 通过 CDP 真机断言：URL 协议、标题、三个页签、图鉴 10 卡、
实例 6 卡且 6 张参考图 `naturalWidth > 0`、载入流水灯预设播放 12 秒灯态 ≥ 3 种组合、
